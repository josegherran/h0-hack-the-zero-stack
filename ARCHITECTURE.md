...
A single Lambda function triggered by SQS events. It is the only component that calls OpenAI for analysis (not chat). Its execution is:
...
6. Call OpenAI GPT-4o with the structured analysis prompt
...
The `/api/chat` route uses the Vercel AI SDK's `streamText` function to stream GPT-4o responses to the browser via SSE (`text/event-stream`). The contract's extracted text is injected as context in the system prompt, scoped to the authenticated user's organization.
...
**Context:** OpenAI GPT-4o analysis of a 30-page contract takes 15–30 seconds. Vercel API Routes have a maximum execution time of 60 seconds (Pro plan) or 10 seconds (Hobby). Running the analysis synchronously in a Vercel function is fragile and blocks the request.
...
**Context:** The "Chat with the contract" feature requires streaming LLM responses to the browser. Options: raw OpenAI streaming, Vercel AI SDK, or a custom SSE implementation.

**Decision:** Use the Vercel AI SDK's `streamText` function on the server and `useChat` hook on the client.
...
| AI SDK | Vercel AI SDK | 4 | Chat streaming |
| LLM | OpenAI GPT-4o | — | Contract analysis + chat |
...
| **Fine-tuned model** | Fine-tune a smaller model on contract analysis data to reduce cost and latency vs GPT-4o | When analysis volume makes OpenAI costs significant |
...
| Least-privilege IAM | ✅ | Lambda role: `s3:GetObject`, `sqs:ReceiveMessage`, `ses:SendEmail`, Aurora Data API only |
...
| Chat first token | < 1s | Vercel AI SDK streaming; GPT-4o low latency |
...
*ContractLens ARCHITECTURE v1.0.0 — H01 Hackathon (h01.devpost.com) — June 2026*
